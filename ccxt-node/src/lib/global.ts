...
export async function getCoinGlassData({
    ...
    return async function (fundingRates: FundingRates, coins: string[], nextFundingHour: number): Promise<FundingRates> {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (coins.indexOf(symbol) == -1) continue;
...
function processFundingRatesChains(processinglinks: FundingRatesChainFunction[]) {
    return async function ({
        ...
        for (let i = 0; i < processinglinks.length; i++) {
            ...
            fundingRates = await link(fundingRates, coins, nextFundingHour);
        }
        return fundingRates;
    };
}
...
export async function calculateBestRoiTradePair({
    ...
    nextFundingHour: number,
    investment: number,
    processinglinks: FundingRatesChainFunction[],
    referenceData: TradePairReferenceData
}): Promise<RoiTradePair | undefined> {
    let bestTradePair: RoiTradePair | undefined = undefined;

    let fundingRates = await (processFundingRatesChains(processinglinks))({ coins, nextFundingHour });
    ...
    for (let coinIndex = 0; coinIndex < coins.length; coinIndex++) {
        ...
        for (let exchangeIndex = 0; exchangeIndex < exchanges.length; exchangeIndex++) {
            ...
            for (let pairIndex = 0; pairIndex < pairs.length; pairIndex++) {
                ...
                let longLeverage: number = 1;
                let maxLeverage: number = 1;
                let rate: number = 0;
                let riskIndex: string = '';
                let shortLeverage: number = 1;

                if (type == 'margin') {
                    maxLeverage = (reference.marginMaxLeverage || 1);

                    let leveragedValue = maxLeverage * investmentInLeg;
                    longLeverage = leveragedValue > (reference.quoteBorrowLimit || 1) ?
                        (reference.quoteBorrowLimit || 1) / investmentInLeg : maxLeverage;

                    let latestPrice = (await exchange.fetchOHLCV(symbol, undefined, undefined, 1))[0][4];
                    let baseBorrowLimit = (reference.baseBorrowLimit || 1) * latestPrice;
                    shortLeverage = leveragedValue > baseBorrowLimit ?
                        baseBorrowLimit / investmentInLeg : maxLeverage;
                }
                else {
                    rate = fundingRates[coin][exchangeName][symbol];
                ...
                let coinCalculation: FundingRateCalculation = {
                    ...
                    symbol,
                    longFee: reference.longFee || 0,
                    shortFee: reference.shortFee || 0,
