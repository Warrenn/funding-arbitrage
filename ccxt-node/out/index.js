...
import { factory, calculateBestRoiTradingPairs, processFundingRatesPipeline, getCoinGlassData } from './lib/global.js';
dotenv.config({ override: true });
const apiCredentialsKeyPrefix = `${process.env.API_CRED_KEY_PREFIX}`, tradeStatusKey = `${process.env.TRADE_STATUS_KEY}`, coinglassSecretKey = `${process.env.COINGLASS_SECRET_KEY}`, region = `${process.env.CCXT_NODE_REGION}`, refDataFile = `${process.env.REF_DATA_FILE}`;
...
let referenceData = JSON.parse(await fs.promises.readFile(filePath, { encoding: 'utf8' }));
...
while (true) {
    ...
    if (tradingState.state == 'closed' && currentHour >= nextOnboardingHour) {
        ...
        let fundingRates = await processFundingRatesPipeline([
            ...
            poloniexRates,
            xtRates
        */ /*])({ nextFundingHour: nextTradingHour });
let tradePairs = await calculateBestRoiTradingPairs({
fundingRates,
exchangeCache,
investment: 1000,
