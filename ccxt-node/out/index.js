...
settings.investmentMargin = 0.99;
    ...
    while (true) {
        try {
            ...
            if (tradingState.state == 'closed' && currentHour >= nextOnboardingHour) {
                if (!logMessage)
                    logMessage = `calculating next trade for ${nextTradingHour}`;
                if (logMessage)
                ...
                if (longPrecision <= 1 && longPrecision > 0)
                    orderSize = Math.floor(orderSize / longPrecision) * longPrecision;
                if (shortPrecision <= 1 && shortPrecision > 0)
            ...
            if (tradingState.state == 'open' && tradingState.fundingHour == nextTradingHour) {
                ...
                let longFunding = Math.floor(((((_b = longBalace[longDetails.currency]) === null || _b === void 0 ? void 0 : _b.free) || 0) - (longExchange.options.leaveBehind || 1)) * 100) / 100;
                if (longFunding > 0) {
                ...
                let shortFunding = Math.floor(((((_c = shortBalace[shortDetails.currency]) === null || _c === void 0 ? void 0 : _c.free) || 0) - (shortExchange.options.leaveBehind || 1)) * 100) / 100;
                if (shortFunding > 0) {
